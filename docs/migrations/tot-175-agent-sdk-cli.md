...
`CodingAgent.open` now returns a configured `Cli`. Each node builds a `Prompt`, selects an optional opaque `Continuation`, calls `Cli.start()` or `Cli.continue_session()` exactly once when acquiring its scoped resource, invokes `CliSession.prompt`, and decodes the resulting `FinalResponse`. Codex forwards caller-ordered context-file paths; OpenCode resolves relative paths against the workspace before constructing the common prompt. A node owns a mutex for prompt serialization and a no-op resource finalizer because agent-sdk exposes no idle close operation. Prompt cancellation is cleaned up by the provider and then re-raised. `Continuation` is in-process only: it cannot be serialized, persisted, or shared with another agent.
