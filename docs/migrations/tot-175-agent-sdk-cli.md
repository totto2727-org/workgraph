...
The public versions are `totto2727/agent-sdk@0.2.1`, `totto2727/codex-sdk@0.4.1`, and `totto2727/opencode-sdk@0.4.1`; `workgraph-agent-cli`, `workgraph-codex-cli`, and `workgraph-opencode-cli` are version `0.2.1`. CI resolves published dependencies from the MoonBit registry and never rewrites `moon.work` with source checkouts or overlays. The [shared Nix setup and MoonBit actions](https://github.com/totto2727-org/monorepo/tree/main/.github/actions) run without an explicit target so each module's preferred target governs validation, while the `ci` Nix dev shell supplies the Codex and OpenCode executables.
