...
1. core、coding-agent、visualizationの実行はJavaScript、native、Wasm/WASIをサポートし、LLMの実行はJavaScriptとnativeをサポートします。CodexとOpenCodeのCLI integrationは同じproduction sourceでWasm/WASIとnativeをサポートします。
2. グラフの実行は逐次的です。
