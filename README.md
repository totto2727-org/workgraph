...
`workgraph-agent-cli`, `workgraph-codex-cli`, and `workgraph-opencode-cli` are version `0.2.1`, prefer Wasm/WASI, and support Wasm/WASI and native from the same production source; they do not support JavaScript. Provider integrations compose registry-resolved `totto2727/agent-sdk@0.2.1` with `totto2727/codex-sdk@0.4.1` or `totto2727/opencode-sdk@0.4.1`. CI uses the shared MoonBit setup and check actions without an explicit target, so each module's preferred target governs validation. The `ci` Nix dev shell extends the default MoonBit and Node.js shell with the Codex and OpenCode CLIs; the default shell remains unchanged. Provider SDK imports are retained only for provider-native option types exposed by each Workgraph composition root. Process-backed fake CLI tests run only on native; a real credentialed smoke remains optional. See the [Agent CLI SDK migration note](docs/migrations/tot-175-agent-sdk-cli.md) for the breaking API and target change.
